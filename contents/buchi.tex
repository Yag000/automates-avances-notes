...
\begin{proof}
	...
	\begin{eqnarray*}
		...
		\eta_1 &=& \setdef {(f_1,a,q)} {(f_2,a,q) \in \delta_2} \\
...
\begin{definition}
	...
	$$\larr L = \setdef {w \in \alphabet^ {\omega}} {w \text{ a une infinité de prefixes dans } L}$$
\end{definition}
...
\begin{proof}
	Par l'absurde, si un automate déterministe $\exists  L \in \text{Rat}(\alphabet)$ \tq tout mot de $X$ a un nombre infini de préfixes dans $L$.
	...
	qui est l'étiquette d'un chemin qui passe un nombre infini de fois par un état final. Donc ce mot serait reconnu pat l'automate, or il
	contient un nombre infini de $b$.  $\contradict$
\end{proof}
