...
en suivant un algorithme à complexité polynomiale \quotes{raisonnables}.
...
\begin{itemize}
	\item Les colonnes de $T$ qui sont indexées par un ensemble $E$ de "mots" "clos par suffixes".
	\item Les lignes sont indexées par un ensemble de "mots" $S \cup S\cdot \alphabet$, ou $S$ est
	      un ensemble "clos par préfixes".
\end{itemize}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Supposons l'énoncé vrai pour tout "mot" appartenant à $S \cup S\alphabet$ de longueur inférieure ou égale a $n$ et
		      ...
		      si $t \in S\alphabet$, $s \in S$ trivialement et si $t \in S$, comme $S$ est clos par préfixes, $s \in S$.
