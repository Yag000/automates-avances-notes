...
Montrons que $(\N, + , 0)$ est librement engendré sur $\set 1$.
	...
	Alors on a que $f(1) = m \in M$ et donc $f(n) =  f(\underbrace{1+\ldots+1}_{n \text{f ois}}) = f(1)\cdot \ldots \cdot f(1) = m\cdot \ldots \cdot m = m^n$
    Donc, si on pose $i: \set 1 \to (\N, + 0)$ \tq $f(1) = 1$, alors quelque soit le choix de $m$, \ie, quelque soit la fonction $f$ choisie (car elle dépend juste de $m$), alors 
