...
\digraph[scale=0.5]{langage}{
	...
	Node2 [shape=ellipse, style=filled, color=lightgreen, label=<Automates finis<br/>non déterministes<br/>+ epsilon-transitions>];
...
\begin{definition}[lecture d'un mot par un AFN]
	...
	$$ \begin{array}{rcl}
			\kleene \delta : P(Q) \times \mots & \to     & \parts Q                                                  \\
...
\subsection{Équivalence entre expressions rationnelles et automates finis}

\begin{theorem}[Injection des expressions rationnelles vers les automates finis]
	Soit $r \in \erat$, alors il existe un automate fini déterministe $N(r)$ \tq $\lang A = \lang r$.
\end{theorem}

\begin{proof}
	Nous allons construire un tel automate par induction sur la structure de $r$ cependant la preuve est laissée en exercice. Cette construction est appelée \textbf{construction de Thompson}.
	...
	En combinant ces constructions, on peut construire un automate fini non déterministe pour n'importe quelle expression rationnelle.
