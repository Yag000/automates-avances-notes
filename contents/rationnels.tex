...
\begin{remarque}
	C'est n'est pas vrai pour les non déterministes.
...
L'automate des résiduels déterministe est l'automate minimal reconnaissant.


Nous allons voir toris algorithmes de minimisation différents:
\begin{itemize}
...
\begin{prop}
	Si $A$ est un automate déterministe et accesible, \ie, $\forall q \in Q, \exists \text{ un chemin } q_0 \ to q$,
	et  $\mirror A = \detA (\mirr (A)) $ est l'automate obtenu en determinisant l'automate miroir de $A$. Alors
...
\begin{proof}
	...
	Soit $p$ un état de $T u$. Puisque $A$ est accesible, il existe un chemin de $q_0 \to q$ dans $A$, donc il existe un chemin $p \to q_o$ dans $\mirr A$.
...
\begin{coro}
	Soit $A$ un automate, alors l'automate $\detA (\mirr (\detA(\mirr (A))))$ est l'automate minimal pour $\mirror {\lang A}$.
\end{coro}
...
\begin{prop}
	Si $A$ est un automate déterministe et $\sim$ une congruence sur $A$m alors $\lang A = \lang {A/\sim}$
\end{prop}
...
où $\deltaS q w \notin T \et \deltaS {q'} w \in T$ son deux états non équivalent qui sont dits \emph{séparables}.
Si $q \et q'$ sont séparables et $w$ est un moi qui satisfait \ref{eq:congnot}, on dira que $w$ \emph{sépare} $q$ et $q'$.
...
\begin{definition}[Algorithme de Moore]
	L'algorithme consiste a créer un automate a partir des classes d'équivalence sur $\cong$.
	Les états sont les classes d'équivalence, les transitions on les obtient en regardant le comportement
	d'un représentant de la classe et les états finaux sont les classes qui ont un représentant qui un état final.
\end{definition}
